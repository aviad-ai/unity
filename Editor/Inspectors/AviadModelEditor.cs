...
namespace Aviad
{
    [CustomEditor(typeof(AviadModel))]
    public class AviadModelEditor : UnityEditor.Editor
    {
        private AviadModel model;
        private bool isDownloading = false;

        private void OnEnable()
        {
            model = (AviadModel)target;
        }
        ...
        private IEnumerator DownloadModelEditorCoroutine()
        {
            ...
            string hash = AviadModelRuntime.GetHash(url);
            ...
            {
                AviadLogger.Error($"Download failed: {request.error}");
            }
            DeleteFileIfExists(tempPath);
            isDownloading = false;
        }
        ...
        private void DeleteFileIfExists(string path)
        {
            ...
            {
                AviadLogger.Warning($"Failed to delete file {path}: {ex.Message}");
            }
        }
    }
}