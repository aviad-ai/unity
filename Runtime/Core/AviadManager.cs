...
namespace Aviad
{
    public static class AviadManager
    {
        ...
        public static bool EnableNativeLogging => AviadGlobalSettings.IsNativeLoggingEnabled;
        ...
        public static void Initialize(Action<bool> callback = null)
        {
            ...
            try
            {
                InternalInitialize(success =>
                {
                    ...
                    foreach (var cb in callbacks)
                    {
                        ...
                        {
                            AviadLogger.Error($"[AviadManager] Init callback error: {ex}");
                        }
                    }
                });
            }
            ...
            {
                AviadLogger.Error($"[AviadInteropManager] Failed to initialize AviadGeneration instance: {ex}");
                lock (_initLock)
                {
                    _isInitializing = false;
        ...
        private static void MaybeEnableLogging()
        {
            if (EnableNativeLogging && _instance != null)
            {
                ...
                {
                    AviadLogger.Debug("[AviadManager] Logging setup success: " + success);
                });
            }
        }
...
#else
            var rawInstance = new AviadBackgroundNativeLibrary(new AviadNativeLibrary());
