...
namespace Aviad
{
    ...
    public class Cluster
    {
        ...
        public void AddContributor(string input, float[] embedding)
        {
            if (embedding == null)
            {
                PackageLogger.Warning("Contributor embedding cannot be null.");
                return;
            }
        ...
        public float[] GetCentroid()
        {
            ...
            {
                PackageLogger.Warning("Cannot compute centroid: no contributors in cluster.");
                return null;
            }
            ...
            {
                PackageLogger.Warning("Cannot compute centroid: first contributor has null embedding.");
                return null;
            }
            ...
            foreach (var contributor in contributors)
            {
                ...
                {
                    PackageLogger.Warning("Skipping contributor with null embedding in centroid calculation.");
                    continue;
                }

                if (contributor.Embedding.Length != dimensions)
                {
                    PackageLogger.Warning("Skipping contributor with mismatched embedding dimensions in centroid calculation.");
                    continue;
                }
    ...
    public class Clusters : MonoBehaviour
    {
        ...
        public void AddCluster(string tag)
        {
            ...
            {
                PackageLogger.Error("Tag cannot be null or empty");
                return;
            }

            if (clusters.Any(c => c.Tag == tag))
            {
                PackageLogger.Warning($"Cluster with tag '{tag}' already exists");
                return;
            }

            clusters.Add(new Cluster(tag));
            PackageLogger.Debug($"Added cluster: {tag}");
        }
        ...
        public void AddContributor(string clusterTag, string input, float[] embedding)
        {
            ...
            if (cluster == null)
            {
                PackageLogger.Error($"Cluster with tag '{clusterTag}' not found");
                return;
            }

            if (embedding == null || embedding.Length == 0)
            {
                PackageLogger.Error("Embedding cannot be null or empty");
                return;
            }

            cluster.AddContributor(input, embedding);

            PackageLogger.Debug($"Added contributor to cluster '{clusterTag}': {input}");
        }
        ...
        private static float CosineSimilarity(float[] vector1, float[] vector2)
        {
            ...
            {
                PackageLogger.Warning("Cannot compute cosine similarity: one or both vectors are null.");
                return 0f;
            }

            if (vector1.Length != vector2.Length)
            {
                PackageLogger.Warning("Cannot compute cosine similarity: vectors have different dimensions.");
                return 0f;
            }
            ...
            {
                PackageLogger.Warning("Cannot compute cosine similarity: one or both vectors have zero magnitude.");
                return 0f;
            }
        ...
        private static float EuclideanDistance(float[] vector1, float[] vector2)
        {
            if (vector1 == null || vector2 == null)
            {
                PackageLogger.Warning("Cannot compute Euclidean distance: one or both vectors are null.");
                return float.MaxValue;
            }

            if (vector1.Length != vector2.Length)
            {
                PackageLogger.Warning("Cannot compute Euclidean distance: vectors have different dimensions.");
                return float.MaxValue;
            }
        ...
        public Dictionary<string, float> ComputeProbabilities(float[] inputEmbedding)
        {
            ...
            {
                PackageLogger.Warning("Cannot compute probabilities: input embedding is null.");
                return new Dictionary<string, float>();
            }

            if (clusters.Count == 0)
            {
                PackageLogger.Warning("Cannot compute probabilities: no clusters available.");
                return new Dictionary<string, float>();
            }
            ...
            foreach (var cluster in clusters)
            {
                ...
                if (centroid == null)
                {
                    PackageLogger.Warning($"Skipping cluster '{cluster.Tag}': centroid is null.");
                    continue;
                }
        ...
        private static Dictionary<string, float> Softmax(Dictionary<string, float> similarities)
        {
            ...
            {
                PackageLogger.Warning("Cannot apply softmax: similarities dictionary is null or empty.");
                return new Dictionary<string, float>();
            }
            ...
            if (sum == 0f)
            {
                PackageLogger.Warning("Softmax sum is zero, returning uniform probabilities.");
        ...
        public string Predict(float[] inputEmbedding)
        {
            ...
            if (prediction != null)
            {
                PackageLogger.Debug($"Prediction: {prediction} (probability: {probabilities[prediction]:F3})");
            }
        ...
        public bool RemoveCluster(string tag)
        {
            ...
            if (cluster != null)
            {
                ...
                PackageLogger.Debug($"Removed cluster: {tag}");
                return true;
            }
            return false;
        }
        ...
        public bool RemoveContributor(string clusterTag, int contributorIndex)
        {
            ...
            {
                PackageLogger.Error($"Invalid contributor index {contributorIndex} for cluster '{clusterTag}'. Valid range: 0-{cluster.Contributors.Count - 1}");
                return false;
            }
            ...
            PackageLogger.Debug($"Removed contributor from cluster '{clusterTag}': {contributor.Input}");
            return true;
        }
    }
}