...
//#if UNITY_WEBGL && !UNITY_EDITOR
    public class GenerationWebgl : ITextGeneration, IWebGLUtilities
    {
        ...
        private static void OnCallback(string callbackId, string message)
        {
            ...
            {
                PackageLogger.Warning($"Unknown callback ID: {callbackId}");
                return;
            }
            ...
            switch (returnType)
            {
                case CallbackReturnType.Bool:
                    if (_boolCallbacks.TryGetValue(callbackId, out var boolCallback))
                    {
                        ...
                        {
                            PackageLogger.Warning($"Failed to parse '{message}' as bool for callback ID: {callbackId}");
                        }
                    }
                ...
                case CallbackReturnType.LlamaMessageSequence:
                    if (_msgSeqCallbacks.TryGetValue(callbackId, out var msgSeqCallback))
                    {
                        ...
                        {
                            PackageLogger.Warning($"Failed to parse '{message}' as LlamaMessageSequence for callback ID: {callbackId}. {e.Message}");
                        }
                    }
                ...
                default:
                    PackageLogger.Warning($"Unhandled return type '{returnType}' for callback ID: {callbackId}");
                    break;
            }
        ...
        public void SetLoggingEnabled(Action<bool> onDone = null)
        {
            ...
            {
                PackageLogger.Error($"[AviadGenerationWebgl] SetLoggingEnabled failed: {ex.Message}");
                onDone?.Invoke(false);
            }
        }
            ...
            Action<LlamaMessageSequence> onResult = null)
        {
            ...
            {
                PackageLogger.Error($"GetContext failed: {ex.Message}");
            ...
            Action<bool> onDone = null)
        {
            ...
            {
                PackageLogger.Error($"InitContext failed: {ex.Message}");
                onDone?.Invoke(false);
            }
        }
            ...
            Action<bool> onDone = null)
        {
            ...
            {
                PackageLogger.Error($"AddTurnToContext failed: {ex.Message}");
                onDone?.Invoke(false);
            }
        }
        ...
        public void AppendToContext(string contextKey, string content, Action<bool> onDone = null)
        {
            ...
            {
                PackageLogger.Error($"AppendToContext failed: {ex.Message}");
                onDone?.Invoke(false);
            }
        }
        ...
        public void CopyContext(string sourceContextKey, string targetContextKey, Action<bool> onDone = null)
        {
            ...
            {
                PackageLogger.Error($"AviadCopyContext failed: {ex.Message}");
                onDone?.Invoke(false);
            }
        }
        ...
        public void FreeContext(string contextKey, Action<bool> onDone = null)
        {
            ...
            {
                PackageLogger.Error($"FreeContext failed: {ex.Message}");
                onDone?.Invoke(false);
            }
        }
        ...
        public void InitializeModel(string modelId, LlamaInitializationParams modelParams, Action<bool> onComplete)
        {
            ...
            {
                PackageLogger.Error($"InitializeModel failed: {ex.Message}");
                _activeModelIds.Remove(modelId);
        ...
        public void AbortInitializeModel(string modelId, Action<bool> onComplete)
        {
            ...
            {
                PackageLogger.Error($"AbortInitializeModel failed: {ex.Message}");
                _activeModelIds.Remove(modelId);
                onComplete?.Invoke(false);
            }
        }
        ...
        public void ShutdownModel(string modelId, Action<bool> onComplete)
        {
            ...
            {
                PackageLogger.Error($"ShutdownModel failed: {ex.Message}");
                _activeModelIds.Remove(modelId);
                onComplete?.Invoke(false);
            }
        }
        ...
        public void UnloadActiveContext(string modelId, Action<bool> onDone = null)
        {
            ...
            {
                PackageLogger.Error($"UnloadActiveContext failed: {ex.Message}");
                onDone?.Invoke(false);
            }
        }
            ...
            Action<bool> onDone = null)
        {
            ...
            {
                PackageLogger.Error($"LoadContext failed: {ex.Message}");
                onDone?.Invoke(false);
            }
        }
        ...
        public void CacheContext(string modelId, Action<bool> onDone = null)
        {
            ...
            {
                PackageLogger.Error($"CacheContext failed: {ex.Message}");
                onDone?.Invoke(false);
            }
        }
            ...
            Action<bool> onDone)
        {
            ...
            {
                PackageLogger.Error($"GenerateResponse failed: {ex.Message}");
                onDone?.Invoke(false);
            }
        }
        ...
        public void AbortGeneration(string modelId, Action<bool> onComplete)
        {
            ...
            {
                PackageLogger.Error($"AbortGeneration failed: {ex.Message}");
                onComplete?.Invoke(false);
            }
        }
            ...
            Action<float[]> onComplete)
        {
            try
            {
                ...
                AviadComputeEmbeddings(modelId, context, embeddingParams.ToJson(), AddStringCallback(result =>
                {
                    ...
                    {
                        PackageLogger.Error($"Failed to parse embeddings result: {ex.Message}");
        ...
        public void GetEmbeddingsSize(string modelId, Action<uint> onComplete)
        {
            try
            {
                AviadGetEmbeddingsSize(modelId, AddStringCallback(result =>
                {
                    ...
                    {
                        PackageLogger.Error($"Failed to parse embeddings size: {result}");
                        onComplete?.Invoke(0);
                    }
                }));
            }
        ...
        public void DownloadFile(string url, string targetPath, Action<bool> onComplete)
        {
            ...
            {
                PackageLogger.Error($"DownloadFile failed: {ex.Message}");
                onComplete?.Invoke(false);
            }
        }
        ...
        public void Start(Action<bool> onComplete)
        {
            ...
            {
                PackageLogger.Error($"Start failed: {ex.Message}");
                onComplete?.Invoke(false);
            }
        }
        ...
        public void Dispose()
        {
            PackageLogger.Debug($"Cleanup started. Active models: {_activeModelIds.Count}");

            if (_activeModelIds.Count == 0)
            {
                PackageLogger.Debug("No active models to cleanup.");
                return;
            }
            ...
            foreach (var modelId in modelsToShutdown)
            {
                ...
                {
                    PackageLogger.DebugFormat("Model {0} shutdown. Success: {1}", modelId,
                        success);
                    remainingModels--;

                    if (remainingModels == 0)
                    {
                        PackageLogger.Debug("All models have been shutdown.");
